...
// camera 2,3,4
Camera camera2(glm::vec3(2.5f, 0.0f, 0.0f));
Camera camera3(glm::vec3(0.01f, 2.5f, 0.0f));
Camera camera4(glm::vec3(0.0f, 0.0f, 2.5f));
...
int main()
{
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * 999, &translations[0], GL_STATIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, 0);

    unsigned int instanceVBO2;
    glGenBuffers(1, &instanceVBO2);
    glBindBuffer(GL_ARRAY_BUFFER, instanceVBO2);
    glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec4) * 999, &rotations[0], GL_STATIC_DRAW);
    ...
    makeSphere(0.05f, 32, 32, vertices, normals, indices);
    ...
    do {
        ...
        glBindVertexArray(quadVAO);
        glDrawArraysInstanced(GL_TRIANGLES, 0, 3, 1000); // 1000 triangles of 6 vertices each
        glBindVertexArray(0);
        ...
        for(int i = 0; i < triangles.size(); i++)
        {
            ...
            if (glm::distance2(closestPoint, sphereMove) < 0.0025)
            {
